...
`kernel/tests/goldens/` and `surfaces/tests/goldens/` were generated by the
original Python kernel and are frozen as the parity contract (1e-12 relative
tolerance in `kernel/tests/parity.rs`). The export scripts lived in `tools/`
and require the Python codebase — check out the `python-final` tag to
regenerate.
